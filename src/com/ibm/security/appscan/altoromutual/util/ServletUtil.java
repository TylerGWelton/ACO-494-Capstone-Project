...
public class ServletUtil {
	...
	public static final Pattern XSS_REGEXP = Pattern.compile(
	 ...
	 * @param path  path to the news XML file
	...
	public static String[] searchArticles(String query, String path) {
		...
		try {
			...
			factory.setFeature("http://apache.org/xml/features/disallow-doctype-decl", true); // this disallows doctype
																								// delc
			factory.setFeature("http://xml.org/sax/features/external-general-entities", false); // this disallows
																								// external ents
			factory.setFeature("http://xml.org/sax/features/external-parameter-entities", false); // this disallows
																									// external perams
			factory.setFeature("http://apache.org/xml/features/nonvalidating/load-external-dtd", false); // this
																											// disallows
																											// loading
																											// external
																											// dtds
	...
	/*
	 * Recursive method to search files in a directory
	 */
	...
	/*
	 * retrieves demo properties to extend AltoroJ behavior for protected
	 * environments
	 */
	...
	/*
	 * Retrieves app property and returns "true" if the app property is set to
	 ...
	 * 
	 * @param feedbackId feedback entry to retrieve
	 */
	...
	static public void initializeRestAPI(ServletContext servletContext) {
		...
		try {
			...
			BufferedWriter swaggerWriter = new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream(servletContext.getRealPath("swagger/properties.json"))));
			swaggerWriter.write(swaggerProps.toString());
			swaggerWriter.close();
		} catch (Exception e) {
			Log4AltoroJ.getInstance().logError("Can't automatically initialize Swagger properties.\n "
					...
					+ "For example: /AltoroJ/api if AltoroJ index page is at http://appscanvm/AltoroJ/index.html \n"
